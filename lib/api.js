...
API.prototype._processTxps = function(txps) {
  ...
  _.each([].concat(txps), function(txp) {
    ...
    _.each(txp.actions, function(action) {
      action.zelerName = API._decryptMessage(action.zelerName, encryptingKey);
      action.comment = API._decryptMessage(action.comment, encryptingKey);
      // TODO get zelerName from Credentials -> zelerId to zelerName
      // action.zelerName = null;
    });
...
API.prototype._import = function(cb) {
  ...
  self.openWallet(function(err, ret) {
    ...
    // Is the error other than "zeler was not found"? || or no priv key.
...
API.prototype.openWallet = function(cb) {
  ...
  self._doGetRequest('/v2/wallets/?includeExtendedInfo=1', function(err, ret) {
    ...
    if (!self.credentials.hasWalletInfo()) {
      var me = _.find(wallet.zelers, {
        id: self.credentials.zelerId
      });
    ...
    if (self.credentials.walletPrivKey) {
      if (!Verifier.checkCopayers(self.credentials, wallet.zelers)) {
        return cb(new Errors.SERVER_COMPROMISED);
      }
    } else {
      // this should only happen in AIR-GAPPED flows
      log.warn('Could not verify zelers key (missing wallet Private Key)');
    }

    self.credentials.addPublicKeyRing(API._extractPublicKeyRing(wallet.zelers));
...
API.prototype._doRequest = function(method, url, args, useSession, cb) {
  ...
  if (self.credentials) {
    headers['x-identity'] = self.credentials.zelerId;
 ...
 * @param {String} zelerName
 * @param {Object} Optional args
 * @param {String} opts.customData
 * @param {String} opts.coin
 * @param {Callback} cb
 */
API.prototype._doJoinWallet = function(walletId, walletPrivKey, xPubKey, requestPubKey, zelerName, opts, cb) {
  $.shouldBeFunction(cb);
  var self = this;
  ...
  var encCopayerName = Utils.encryptMessage(zelerName, this.credentials.sharedEncryptingKey);
  ...
  args.zelerSignature = Utils.signMessage(hash, walletPrivKey);

  var url = '/v2/wallets/' + walletId + '/zelers';
...
API._extractPublicKeyRing = function(zelers) {
  return _.map(zelers, function(zeler) {
    var pkr = _.pick(zeler, ['xPubKey', 'requestPubKey']);
    pkr.zelerName = zeler.name;
    return pkr;
  });
};
 ...
 *
 * @param {string} coin - 'zel' (default) or 'bch'
 ...
 * @param {string} opts.coin[='zel'] - The coin for this wallet (zel, bch).
 ...
 * @return {undefined}
 */
API.prototype.createWallet = function(walletName, zelerName, m, n, opts, cb) {
  var self = this;
  ...
  self._doPostRequest('/v2/wallets/', args, function(err, res) {
    ...
    c.addWalletInfo(walletId, walletName, m, n, zelerName);
    var secret = API._buildSecret(c.walletId, c.walletPrivKey, c.coin, c.network);

    self._doJoinWallet(walletId, walletPrivKey, c.xPubKey, c.requestPubKey, zelerName, {
        coin: coin
      },
      function(err, wallet) {
        if (err) return cb(err);
 ...
 * @param {String} zelerName
 * @param {Object} opts
 * @param {string} opts.coin[='zel'] - The expected coin for this wallet (zel, bch).
 * @param {Boolean} opts.dryRun[=false] - Simulate wallet join
 * @param {Callback} cb
 * @returns {Callback} cb - Returns the wallet
 */
API.prototype.joinWallet = function(secret, zelerName, opts, cb) {
  var self = this;
  ...
  self._doJoinWallet(secretData.walletId, secretData.walletPrivKey, self.credentials.xPubKey, self.credentials.requestPubKey, zelerName, {
    coin: coin,
    dryRun: !!opts.dryRun,
  }, function(err, wallet) {
    if (err) return cb(err);
    if (!opts.dryRun) {
      self.credentials.addWalletInfo(wallet.id, wallet.name, wallet.m, wallet.n, zelerName);
    }
  ...
  this.getStatus({
    ...
    self._doPostRequest('/v2/wallets/', args, function(err, body) {
      ...
      async.each(self.credentials.publicKeyRing, function(item, next) {
        var name = item.zelerName || ('zeler ' + i++);
...
API.prototype._processWallet = function(wallet) {
  ...
  _.each(wallet.zelers, function(zeler) {
    var name = Utils.decryptMessage(zeler.name, encryptingKey);
    if (name != zeler.name) {
      zeler.encryptedName = zeler.name;
    }
    zeler.name = name;
    _.each(zeler.requestPubKeys, function(access) {
...
API.prototype._processStatus = function(status) {
  ...
  function processCustomData(data) {
    var zelers = data.wallet.zelers;
    if (!zelers) return;

    var me = _.find(zelers, {
      'id': self.credentials.zelerId
    });
 ...
 * @param {String} opts.includeOwn[=false] (optional) - Do not ignore notifications generated by the current zeler
...
API.prototype.getNotifications = function(opts, cb) {
  ...
  self._doGetRequestWithLogin(url, function(err, result) {
    ...
    var notifications = _.filter(result, function(notification) {
      return opts.includeOwn || (notification.creatorId != self.credentials.zelerId);
    });
...
/**
 * Adds access to the current zeler
 * @param {Object} opts
...
API.prototype.addAccess = function(opts, cb) {
  ...
  var zelerId = this.credentials.zelerId;
  ...
  this._doPutRequest('/v1/zelers/' + zelerId + '/', opts, function(err, res) {
    if (err) return cb(err);
...
API.prototype.createWalletFromOldCopay = function(username, password, blob, cb) {
  ...
  if (w.publicKeyRing.zelersExtPubKeys.length != w.opts.totalCopayers)
