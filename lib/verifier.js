 ...
 * @returns {Boolean} true or false
 */
Verifier.checkCopayers = function(credentials, zelers) {
  ...
  if (zelers.length != credentials.n) {
  ...
  var uniq = [];
  var error;
  _.each(zelers, function(zeler) {
    if (error) return;

    if (uniq[zelers.xPubKey]++) {
      log.error('Repeated public keys in server response');
      error = true;
    }

    // Not signed pub keys
    if (!(zeler.encryptedName || zeler.name) || !zeler.xPubKey || !zeler.requestPubKey || !zeler.signature) {
      log.error('Missing zeler fields in server response');
      error = true;
    } else {
      var hash = Utils.getCopayerHash(zeler.encryptedName || zeler.name, zeler.xPubKey, zeler.requestPubKey);
      if (!Utils.verifyMessage(hash, zeler.signature, walletPubKey)) {
  ...
  if (!_.includes(_.map(zelers, 'xPubKey'), credentials.xPubKey)) {
