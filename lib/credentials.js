...
Credentials.prototype._expand = function() {
  ...
  this.zelerId = Utils.xPubToCopayerId(this.coin, this.xPubKey);
  this.publicKeyRing = [{
...
Credentials.prototype.addWalletInfo = function(walletId, walletName, m, n, zelerName) {
...
Credentials.fromOldCopayWallet = function(w) {
  function walletPrivKeyFromOldCopayWallet(w) {
    ...
    var seed = w.publicKeyRing.zelersExtPubKeys.sort().join('');
  ...
  var pkr = _.map(w.publicKeyRing.zelersExtPubKeys, function(xPubStr) {
    ...
    var zelerName = w.publicKeyRing.nicknameFor[pubKey];
    if (isMe) {
      credentials.zelerName = zelerName;
    }
